...
<?php if(Yii::app()->session['user_array']['per34']!='1'){?><a href="http://rdlpk.com/index.php/memberplot/installment_edit?id=<?php echo $_REQUEST['id']; ?>">Edit Installment Plan</a>
...
$gtotalsur=$gtotalsur+$totalduesur;
  ...
  if(Yii::app()->session['user_array']['per34']!='1'){
         echo'<td>';if(!empty($pay['fstatus'])){if($pay['fstatus']=='approved'){ echo '<b style="color:Green;">Verified</b>';}else{echo '<b style="color:red;">'.$pay['fstatus'].'</b>';} echo '/ <a href="splitinstallment?id='.$pay['id'].'">Split</a>';} else{ echo'<a href="installment_up?id='.$pay['id'].'">Edit</a>
...
echo '<td>'.$pay1['paid_date'].'</td>
	 ...
	 if(($lastdue) > 0 and $pay['paid_date']!==$pay1['paid_date']){
