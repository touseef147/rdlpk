...
echo '<tr><td>' . $i . '</td>
 ...
 <td style="text-align:right;">'.number_format($dis).'</td>
  ...
  <td>'.number_format($tpayed).'</td>
   <td><a href="installment_details?id='.$_REQUEST['id'].'&& pid='.$_REQUEST['id'].'">Detail</a>/';
 if(Yii::app()->session['user_array']['per34']!='1'){echo'<a href="discount?id='.$msid.'">Discount</a>';}
        echo'</td>
</tr></tbody></table>  ';
...
?>
 ...
 <?php if(Yii::app()->session['user_array']['per34']!='1'){?>
<div style="float:right;"><a href="plotcharges?id=<?php echo $_REQUEST['id']?> && pid=<?php echo $pro;?>"><h5>Add Charges/Fee</h5></a><a href="plotsurcharge?id=<?php echo $_REQUEST['id']?> && pid=<?php echo $pro;?>"><h5>Add Surcharge</h5> </a>
<a href="charges_list?id=<?php echo $_REQUEST['id']?> && pid=<?php echo $pro;?>"><h5>Charges List</h5> </a>
 </div><?php }?>
