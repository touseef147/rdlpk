...
$new_date = date('d-m-y', $middle); 
    ...
    <h5 style="text-align:left;">Price/Installments: <a style="float:right;" href="<?php echo $home?>/memberplot/payment_detailsc?id=<?php echo $key['plot_id']; ?>&&pid=">Payment Details List</a><br></h5>
   <table class="table table-striped table-new table-bordered">
 <tbody>	
...
<tr><td>     Status:</td><td><?php echo $key['status']?></td></tr>
    ...
    <h4>Forward for Cancellation</h4>
    <?php 
	if(Yii::app()->session['user_array']['per33']==1 && $plotdetails['cpstatus']=='approved'){
...
<div id="error-div" class="errorMessage" style="display: none; color:#F00; font-weight:bold;"></div>
   ...
   <input type="hidden" name="plot_id"  value="<?php echo $_REQUEST['plot_id'];?>" />
	...
	<input type="text" value="" name="fcomment" id="fcomment" class="reg-login-text-field" style="width:240px;" />
     <?php echo CHtml::ajaxSubmitButton('Submit Cancel',array('Fapproved'), array( 'beforeSend' => 'function(){ 
