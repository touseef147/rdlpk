...
<div class="black-bg">Buiilding Name:</div><div class="grey-bg"><input type="text" readonly=readonly() style="height:16px; " name="com_res" id="com_res" value="'.$key['buildname'].'"></div>
<div class="black-bg">Floor Name:</div><div class="grey-bg"><input type="text" readonly=readonly() style="height:16px; " name="com_res" id="com_res" value="'.$key['fname'].'"></div>
<div class="black-bg">Property category:</div><div class="grey-bg"><input type="text" readonly=readonly() style="height:16px; " name="com_res" id="com_res" value="'.$key['com_res'].'"></div>
<div class="span6 left-box">
  <h5 style="text-align:left;">Property Details</h5> 	

  <div class="">

  	

	<div class="black-bg">Property info/Address:</div><div class="grey-bg"><input type="text" readonly=readonly() style="height:16px;" name="plot_detail_address" id="plot_detail_address" value="'.$key['plot_detail_address'].'"></div><br>
...
?>
 <?php 
	$connection = Yii::app()->db; 	
$sql_details2  = "SELECT * FROM property where id=".$_REQUEST['id']."";
$result_details2 = $connection->createCommand($sql_details2)->queryRow();
$sql_details3  = "SELECT * FROM property where id=".$_GET['id'];
...
?>
  ...
  <h5 style="text-align:left;">Payment Details Link:</h5>
    ...
    <a href="prop_payment_details?id=<?php echo $_GET['id'] ?>&pid=<?php echo $key['project_id'] ?>">Payment Detail Link</a></div><br>
   
    </div> 
<?php 
if($result_details3['status']!='Approved'){?>
  <form action="Prop_submitstatus" method="post" enctype="multipart/form-data" onsubmit="return validateForm()" >

    <input type="hidden" value=<?php echo $_REQUEST['id']?> name="plot_id" id="plot_id" class="f-left span4 clearfix" />
