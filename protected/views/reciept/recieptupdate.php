   ...
   $sql_payment1  = "SELECT * FROM plotpayment where r_id='".$data['rid']."'";
	...
	$result_payments2 = $connection->createCommand($sql_payment2)->queryAll();
		foreach($result_payments2 as $row2){
			$n=$n+1;
		$totalp=$totalp+$row2['paidamount']+$row2['paidsurcharge'];}
...
$this->endWidget(); }?>
  ...
  <?php if($data['comm']!==''){?>
  <b style="color:red;">Remarks By Finance</b>:<?php echo $data['comm'];}?> <a style="float:right;" href="updaterecieptotherms?id=<?php echo $_REQUEST['id']?>">For Other MS</a>
...
////////////////////////////////////////////////////END: NORMAL PLOTS (Charges and Installments)////////////////////////////////////////////// 
?>
...
<h5>Property Charges</h5>
  ...
  <table class="table table-striped table-new table-bordered">
    ...
    <tbody>
      <?php  
    $sql_plot1  = "SELECT property.plotno,propertypayment.*,propertypayment.id as cid from propertypayment 
...
$result_plots = $connection->createCommand($sql_plot)->queryAll();
  $sql_t  = "SELECT *,property.id as pid from property
 Left join plots on (plots.id=property.plot_id)
Left join transferplot on (property.id=transferplot.plot_id)
Left join projects on (projects.id=plots.project_id)
Left join streets on (streets.id=plots.street_id)
Left join sectors on (sectors.id=plots.sector)
 where transferplot.transferto_id='".$mem."' and property.mstatus!=2";
$result_t = $connection->createCommand($sql_t)->queryAll();
echo '<option value="">Select Plot</option>';
foreach($result_plots as $po){
	echo '<option value="'.$po['pid'].'">'.$po['plotno'].'/'.$po['mstatus'].'</option>';
...
?>
  ...
  <table class="table table-striped table-new table-bordered">
    ...
    <tbody>
      <?php  $connection = Yii::app()->db; 
    $sql_plot2  = "SELECT proinstallpayment.id as iid,
	proinstallpayment.lab,
	proinstallpayment.due_date,
	proinstallpayment.dueamount,
	proinstallpayment.paidamount,
	proinstallpayment.surcharge,
	proinstallpayment.paidsurcharge,
...
$result_plots2 = $connection->createCommand($sql_plot2)->queryAll();
foreach($result_plots2 as $ins){
 $sql_rpt  = "Select * from rpt_print where rid='".$_REQUEST['id']."'and msid='".$ins['plot_id']."'  ";
$result_rpt = $connection->createCommand($sql_rpt)->queryRow();	
if($ins['dueamount']==''){$ins['dueamount']=0;}
	if($ins['paidamount']==''){$ins['paidamount']=0;}
	if($ins['surcharge']==''){$ins['surcharge']=0;}
	if($ins['paidsurcharge']==''){$ins['paidsurcharge']=0;}
if($ins['ref'] > 0){
$sql_ref  = "Select * from proinstallpayment where id='".$ins['ref']."'";
$result_ref = $connection->createCommand($sql_ref)->queryRow();	
	$ins['lab']=$result_ref['lab'];
	$ins['due_date']=$result_ref['due_date'];
	}
echo '<tr>
<td>'.$ins['plotno'].'</td>
<td>'.$ins['lab'].'</td>
<td>'.$ins['due_date'].'</td>
<td style="text-align:right;">'.number_format($ins['dueamount']).'</td>
<td style="text-align:right;">'.number_format($ins['paidamount']).'</td>
<td style="text-align:right;">'.floatval($ch['surcharge']).'</td>
<td style="text-align:right;">'.number_format($ins['paidsurcharge']).'</td>
<td>'.$ins['remarks'].'</td>
<td>';
if($data['fstatus']!=='Verified' && Yii::app()->session['user_array']['per18']==1){
echo '<a href="delpropins?id='.$ins['iid'].'&&rid='.$_REQUEST['id'].'">Delete</a>';
}
echo '</td>
<td>'.$result_rpt['r_no'].'</td>
</tr>';}
...
<?php if($data['fstatus']!=='Verified' && Yii::app()->session['user_array']['per18']==1){?>
<a class="btn" href="Genraterecp?id=<?php echo $_REQUEST['id']; ?>">Ganerate Receipt</a>
<?php }?>
...
elem.value = newv;
var elem1 = document.getElementById("proppaidch");
///elem1.value = newv;
//$("#plotno").value(newv);
          }
    });
...
foreach($result_plots12 as $new){
	$pages=0;
foreach($result_plots147 as $ch1){	
...
$result_rpt = $connection->createCommand($sql_rpt)->queryRow();	
 $sql_member  = "Select * from members where cnic='".$data['cnic']."'";
$result_member = $connection->createCommand($sql_member)->queryRow();	
 $sql_membership  = "Select * from property
Left Join size_cat_prop on (property.size2=size_cat_prop.id)
 where property.id='".$result_rpt['msid']."'";
$result_membership = $connection->createCommand($sql_membership)->queryRow();	
$total=0;
$totalw='';
$othermember1[]='';
...
foreach($result_plots2 as $ch1){	
 ...
 $othemember  = "Select property.*,members.name from property
left join members on(property.member_id=members.id)
 where property.id='".$new['plot_id']."'";
$othermember1 = $connection->createCommand($othemember)->queryRow();	
if($ch1['plot_id']==$new['plot_id']){
$total=$total+$ch1['paidamount'];}}
foreach($result_plots147 as $ch1){	
...
//echo $ch1['plot_id'];
  ...
  $othemember2  = "Select * from property
left join members on(property.member_id=members.id)
 where property.id='".$new['plot_id']."'";
$othermember2 = $connection->createCommand($othemember2)->queryRow();
if($ch1['plot_id']==$new['plot_id']){
