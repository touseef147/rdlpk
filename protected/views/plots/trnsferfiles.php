<div class="shadow">
  <h3>Summary of Transferred Cases</h3>
</div>
           <?php
...
<section class="reg-section margin-top-30">
  ...
  <div class="float-left"> <span style="float:right"> <strong>Overall Report</strong></span>
...
$sql_plots  = "SELECT DISTINCT size_cat.size_sorting,size2,size FROM plots
	Left JOIN size_cat ON (plots.size2 = size_cat.id) where size_cat.type='Res' order by size_cat.size_sorting ASC ";
...
echo '<td class="bgd" style="text-align:right;"><b>'.number_format($total_no_plotsr).'</b></td>';
 ?>                 </tbody>
            </table><span style="float:right"> <strong>Overall Report</strong></span>
            <table class="table table-striped table-new table-bordered">
            	<thead style="background:#666; border-color:#ccc; color:#fff;">
                    <tr>
                        <th width="10%">Plot Categories</th>
...
$result_streets = $connection->createCommand($sql_street)->query();
	//echo $total_no_plots;exit
$sql_plots  = "SELECT DISTINCT size_cat.size_sorting,size2,size FROM plots
	Left JOIN size_cat ON (plots.size2 = size_cat.id) where size_cat.type='Com' order by size_cat.size_sorting ASC ";
$result_plots = $connection->createCommand($sql_plots)->query();	
foreach($result_plots as $row1)
{
...
echo '<td class="bgd" style="text-align:right;"><b>'.number_format($total_no_plotsc+$total_no_plotsr).'</b></td>'; ?>                 </tbody>
            </table>
         </div>   
