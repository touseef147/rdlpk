...
<div class="shadow">
  ...
  <h3>Property Payment Details<div style="float:right;"><a href="pro_installment_details?id=<?php echo $_REQUEST['id']?> && pid=<?php // echo $_REQUEST['pid']?>"><h5>Installment Details</h5> </a></div>
</h3>
</div>
...
echo '<tr><td>' . $i . '</td>
 ...
 <td style="text-align:right;">'.number_format($dis).'</td>
  ...
  <td>'.number_format($tpayed).'</td>
   <td><a href="pro_installment_details?id='.$_REQUEST['id'].'&& pid='.$_REQUEST['pid'].'">Detail</a></td>
</tr></tbody></table>  ';
...
<td><?php if($mem['image']==''){ echo'';} else{echo'<img src="'.Yii::getPathOfAlias('webroot').'/upload_pic/'.$mem['image'].'" width=100 height=100/>';} }?></td>
  ...
  <tr >
    ...
    <td  style="text-align:left" ><b>Property Details</b>
...
$new_date = date('d-m-Y', $middle); 
	...
	echo $row['fname'].'</br>'; 
			...
			echo '<br>'.'<b>Property No:</b>';
			 echo $row['plot_detail_address'].'</p><br/>';
 	//echo '<b>Date  :</b>' .$new_date.'</br>';
	$price=$row['price'];
	}  ?><br/><br/></td>
