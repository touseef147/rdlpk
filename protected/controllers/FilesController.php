...
////////////////////////history/////////
    ...
    public function actionMember_lis() {
        ...
        if (Yii::app()->session['user_array']['per2'] == '1') {
            ...
            $sql_member = "SELECT mp.member_id,mp.create_date, m.firstname,m.sodowo,m.cnic,p.plot_detail_address, 					p.plot_size,s.street, j.project_name FROM 		             memberplot mp left join members m on mp.member_id=m.id left join plots p on mp.plot_id=p.id left join streets s on p.street_id=s.id left join projects j on        		s.project_id=j.id    $where ";
            $result_members = $connection->createCommand($sql_member)->query();
   ...
   public function actionSearchreq()
	 	{
		if((Yii::app()->session['user_array']['per2']=='1')||(Yii::app()->session['user_array']['per34']=='1'))
			{
		$com_res='';
			$size='';
		$where='';
...
if (isset($_POST['com_res']) && $_POST['com_res']!=""){
				...
				if ($and==true)
				{
					  $where.=" and size_cat.type ='".$_POST['com_res']."'";
				}
				else
				{

					$where.=" size_cat.type ='".$_POST['com_res']."'";
				}
				$and=true;
			}
...
where $where and plots.type='file' limit $start,$limit";  
    ...
    $res=array();
			...
			echo '<tr><td>'.$key['plotno'].'</td><td>'.$key['project_name'].'</td><td>'.$key['street'].'</td><td><a href="'.$home.'/index.php/user/filehistory?id='.$key['id'].'">'.$key['plot_detail_address'].'</a></td><td>'.$key['plot_size'].'</td><td>'.$key['size'].'</td><td>'.$key['com_res'].'</td><td>'.$key['sector_name'].'</td><td>';
                        if(Yii::app()->session['user_array']['per34']=='1'){echo'Convert';}else{
                        echo'<a href="convert2plot?id='.$key['id'].'">Convert</a>';}
                            echo'</td>';
...
where $where";
            ...
            if (isset($_POST['search'])) {
                ...
                foreach ($result_members as $key) {
                    ...
                    if ($key['status'] == '') {
                        echo'<a href="' . $home . '/index.php/memberfile/allotfile?id=' . $key['id'] . '">' . "Allot" . '</a>';
                    } else {
                        echo $key['status'];
                    }echo '</td>
...
where plots.id='" . $_REQUEST['id'] . "' AND type='file'";
    ...
    public function actionUpdate() {
        ...
        if (Yii::app()->session['user_array']['per3'] == '1') {
            ...
            foreach ($result_sq1 as $row2) {
                $add = $row2['plot_detail_address'];
            }
            ...
            if ($add == $_POST['plot_detail_address']) {
                $count = 0;
            }
