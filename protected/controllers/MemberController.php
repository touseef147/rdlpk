...
//            $count++;
//			
//			 echo '<tr><td>'.$i.'</td><td>'.$key['plotno'].'</td>';
//                         if($key['total']>1){
//                             echo'<td>';
//                             
...
//	}
//	}
/////////////////////////////////END Associate members Directory//////////////////////////////////////////	
...
public function actionDashboard()
	...
	{	
		...
		{ 
			...
			$sql = "SELECT  mp.mstatus as stst,mp.member_id,mp.plot_id,mp.status,mp.create_date,m.image,m.phone,p.project_id,m.address,m.name,m.username,m.mem_id,m.sodowo,m.cnic,p.id,p.plot_detail_address,mp.plotno,p.type,p.sector,p.plot_size,siz.size,p.street_id,s.street, j.project_name,sec.sector_name FROM
			 memberplot mp left join members m on mp.member_id=m.id
			  ...
			  left join sectors sec on sec.id=p.sector
			   left join streets s on p.street_id=s.id
			   left join size_cat siz on p.size2=siz.id
			left join projects j on s.project_id=j.id  WHERE m.id = '".$user_data['id']."' and p.type='plot' ";
			$result = $connection->createCommand($sql)->query();
				$rowspt =count($result);
 ...
 left join sectors on f.sector=sectors.id
			...
			left join projects j on s.project_id=j.id WHERE m.id = '".$user_data['id']."' and f.type='file'";
...
$command = $connection -> createCommand($sql);
 $command -> execute(); 
$sqlstatus = "insert into status_history (user_id,status,detail,status_date,plot_id,type) value ("."'".$uid."'".",'".$_POST['msstatus']."','".$_POST['det']."','".date('Y-m-d h:i:s')."','".$_POST['plot_id']."','".$_POST['type']."')"; 		
