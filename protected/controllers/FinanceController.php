...
FROM
    ...
    Left JOIN streets  ON (plots.street_id = streets.id)
	Left JOIN projects  ON (plots.project_id = projects.id)
	Left JOIN property  ON (plots.id = property.plot_id)
where $where";
	...
	{ //echo 123;exit();
		...
		$connection = Yii::app()->db; 
		 $sql_payment  = "SELECT  mp.member_id,mp.plotno,mp.create_date,mp.id as mp_id,p.type,m.name,mp.plotno,m.image,m.sodowo,m.cnic,mp.plot_detail_address as pda,mp.plot_id,mp.status,mp.plot_size,s.street,j.project_name FROM property mp
left join members m on mp.member_id=m.id
left join plots p on mp.plot_id=p.id
left join streets s on p.street_id=s.id
left join projects j on p.project_id=j.id
where $where and mp.status='New'";
		$result_payments = $connection->createCommand($sql_payment)->queryAll();
...
$sql_payment12  = "SELECT * FROM receipt where bank_details !='' and fstatus=''";
			...
			$sql_project = "SELECT * FROM plotpayment where fstatus='' and paidamount!='' and r_id=0 and re_id=0";
			$result_project = $connection->createCommand($sql_project)->query();
			...
			$layout='//layouts/back';
			$sql_payment  = "SELECT  mp.member_id,mp.plotno,mp.create_date,mp.id as mp_id,p.type,m.name,mp.plotno,m.image,m.sodowo,m.cnic,p.plot_detail_address,mp.plot_id,mp.status,p.plot_size,s.street,j.project_name FROM property mp
left join members m on mp.member_id=m.id
left join plots p on mp.plot_id=p.id
left join streets s on p.street_id=s.id
left join projects j on p.project_id=j.id
where mp.fstatus='' and mp.status='New'";
		$result_payments = $connection->createCommand($sql_payment)->queryAll();
			$property=count ($result_payments);
			$this->render('finance',array('payments'=>$payments,'transfer'=>$transfer,'instr'=>count($result_payments12),'allot'=>$allot,'installments'=>$installments,'cancelplot'=>$cancelplot,'property'=>$property));
			}
			else{
				$this->redirect(array('user/dashboard'));
				}
