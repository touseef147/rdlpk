...

{
	...
	{
		if(isset(Yii::app()->session['user_array'])&&(Yii::app()->session['user_array']['per34']!='1'))
			{
			$connection = Yii::app()->db; 	
...
if ((isset($base) && empty($base))){
					...
					$sql  = "INSERT INTO memberplot (app_no,plot_id,user_name,member_id,create_date,noi,insplan,status,plotno) 
	VALUES ('".$_POST['appnoo']."','".$_POST['file_id']."','".$uname."','".$result_data['id']."','".date('Y-m-d H:i:s')."','".$_POST['noi']."','".$_POST['insplan']."','New','".$pn."')";		
        		        $command = $connection -> createCommand($sql);
                        $command -> execute();
