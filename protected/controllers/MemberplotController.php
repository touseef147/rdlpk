...
$result_prime = $connection->createCommand($sql_primeloc)->queryAll();
	...
	{		if((Yii::app()->session['user_array']['per9']=='1')&& isset(Yii::app()->session['user_array']['username']))
			...
			$to=date('d-m-Y');
				...
				if($_POST['status']=='New'){
					$where.="mp.fstatus='approved'";}else{
	...
	$connection = Yii::app()->db; 
    $sql_member = "SELECT  mp.member_id,mp.plotno,mp.create_date,mp.id as mp_id,p.type,m.name,mp.plotno,m.image,m.sodowo,m.cnic,mp.plot_detail_address as pda,mp.plot_id,mp.status,mp.plot_size,s.street,j.project_name FROM property mp
left join members m on mp.member_id=m.id
...
where $where and mp.status='New' and fcomment !=''";
	...
	public function actionPropalotmentreq()
	{
		...
		$connection = Yii::app()->db; 
		echo $sql_payment  = "SELECT  mp.member_id,mp.plotno,mp.create_date,mp.id as mp_id,p.type,m.name,mp.plotno,m.image,m.sodowo,m.cnic,mp.plot_detail_address as pda,mp.plot_id,mp.status,mp.plot_size,s.street,j.project_name FROM property mp
left join members m on mp.member_id=m.id
left join plots p on mp.plot_id=p.id
left join streets s on p.street_id=s.id
left join projects j on p.project_id=j.id
...
//$new_date = date('d-m-Y', $middle);
  ...
  echo '<tr><td>'.$key['plotno'].'</td><td>'.$key['name'].'</td><td>'.$key['sodowo'].'</td><td>'.$key['cnic'].'</td><td>'.$key['pda'].'<td>'.$key['plot_size'].'</td><td>'.$key['street'].'</td><td>'.$key['project_name'].'</td><td><a target="_blank" href="prop_req_detail?id='.$ID.'">Detail</a>
  </td></tr>';
			} 
			}else{echo exit;}
 exit;
...
$result_prime = $connection->createCommand($sql_primeloc)->queryAll();
	...
	{	
	if((Yii::app()->session['user_array']['per33']=='1')&& isset(Yii::app()->session['user_array']['username'])){
			$name='';
			$sodowo='';
			$cnic='';
			$plotno='';
...
$connection = Yii::app()->db; 
echo $sql_memberas = "SELECT cp.status as cpstatus,mp.*,sec.sector_name,s.street,siz.size,p.price,mp.comment,p.com_res,p.plot_detail_address,p.plot_size,pro.project_name, cp.status as cpstatus, mp.plotno,m.name from memberplot mp
...
$check=1;
    $res=array();
            ...
            foreach($result_members as $key){
				...
				<td>';if($key['cpstatus']=='New'){ echo 'Requested (F)';}else {echo'Requested (M)';}echo'</td>
...
$pagination='';
	...
	if((Yii::app()->session['user_array']['per12']=='1') && isset(Yii::app()->session['user_array']['username']))
			{
			$connection = Yii::app()->db; 	
			  $sql_details  = "SELECT cp.detail,cp.type,cp.status,cp.cancel_date,cp.fstatus,mp.*,mp.id as mpid,cp.status as cpstatus,s.street,p.plot_detail_address,p.plot_size,pro.code,sc.code as scode,mp.plotno,mp.tempms,sc.size,mp.comment,se.sector_name,pro.project_name,m.id as mid,m.name,ss.name as ssname,p.com_res,u.email,u.firstname,u.middelname,u.lastname,p.project_id FROM memberplot mp
			Left JOIN members m ON m.id=mp.member_id
...
public function actionCalsur()
	...
	foreach($result_charges1 as $key1){
		...
		if($key1['paid_date']!==$key['paid_date']){
		if($_REQUEST['cdate']==''){$curdate=$key1['paid_date'];}else{$curdate=$_REQUEST['cdate'];}
	$surchargeratio=($key1['paidamount'])/100*0.05;
	$duedate=$key['paid_date'];
	if($_REQUEST['cdate']==''){$paiddate=$key1['paid_date'];}else{$paiddate=$_REQUEST['cdate'];}
	$datetime1 = new DateTime($duedate);
	$datetime2 = new DateTime($paiddate);
...
//	echo $surchargedur;
	...
	if($_REQUEST['cdate']==''){$curdate=$key['paid_date'];}else{$curdate=$key['paid_date'];}
	$surchargeratio=$key['dueamount']/100*0.05;
$duedate=$key['due_date'];
	
	if($_REQUEST['cdate']==''){$paiddate=$key['paid_date'];}else{$paiddate=$_REQUEST['cdate'];}
...
public function actionCalsur123()
	{
	$surchargedur=0;
	$connection = Yii::app()->db;
	$plot_id =$_REQUEST['id'];			
	$sql_charges  = "SELECT * from installpayment where fstatus!='Cancelled' and others !='Cancelled' and plot_id='".$_REQUEST['id']."' and ref =0 and surcharge_re=0 and(paidsurcharge='' or paidsurcharge < 1)";
	$result_charges = $connection->createCommand($sql_charges)->queryAll();	
	$totalssss=0;
	foreach($result_charges as $key){
...
///	echo $totalssss.'</br>'; 
	...
	////////////////START: CALCULATE CHARGES FOR PARTIAL PAYMENT////////
	...
	$result_charges1 = $connection->createCommand($sql_charges1)->queryAll();
	 ...
	 echo $total=$key1['paidamount']/100*.05;
	 /*   $surchargeratio=($key1['paidamount'])/100*0.05+$surchargeratio;*/
	
	$duedate=$key['due_date'];
	if(empty($key1['paid_date'])){  $paiddate=$_REQUEST['datee'];}else{  $paiddate=$key1['paid_date'];}
	...
	//	echo $ratio;
		...
		///}
			////////////////END: CALCULATE CHARGES FOR PARTIAL PAYMENT////////
	}
//	echo round($totalduesur);exit;
    }
...
$sql1t ="SELECT * from transferplot where plot_id=".$key['plot_id']." and (status='New Request' or status='Sales')";
		...
		$result_datat = $connection->createCommand($sql1t)->queryRow();
			...
			<li role="presentation"><a target="_blank" href="download?id='.$key['plot_id'].'">Document</a></li>
			<li role="presentation"><a href="'.Yii::app()->baseUrl.'/index.php/member/ms_status?msid='.$key['msid'].'">Update Status</a></li>';
			if($key['cpstatus']=='New')
			{
				echo'<li role="presentation"><span style="color:red">Cancellation Requested</span></li>';
...
exit;
	...
	{	
	if((Yii::app()->session['user_array']['per2']=='1')&& (Yii::app()->session['user_array']['username'])|| (Yii::app()->session['user_array']['per34']=='1')){
			$name='';
			$sodowo='';
			$cnic='';
			$plotno='';
...
if(isset(Yii::app()->session['user_array']['username']))
  ...
  $sql_payment  = "Select plotpayment.*,members.name,members.id as mid from plotpayment 
			 left join members  on plotpayment.mem_id=members.id
			  where plot_id='".$_REQUEST['id']."' and (plotpayment.mem_id='".$members['member_id']."' or pobm !='' or plotpayment.payment_type NOT IN ('MS Fee','Transfer Fee'))";
			// $sql_payment="Select plotpayment.*,members.name,members.id as mid from plotpayment 
			// left join members  on plotpayment.mem_id=members.id
			 
			// where plot_id='".$_REQUEST['id']."' and (plotpayment.mem_id='".$members['member_id']."' or pobm!='' or payment_type NOT IN ('MS Fee','Transfer Fee')) ";
		$result_payments = $connection->createCommand($sql_payment)->queryAll();
...
where plot_id='".$_REQUEST['id']."'";
		...
		$this->render('charges_list',array('payments'=>$result_payments,'primeloc'=>$result_prime,'landcost'=>$land_cost,'info'=>$result_plotinfo,'receivable'=>$res,'members'=>$result_members));
			}else{
	...
	public function actionUpdatemem(){
		$connection = Yii::app()->db;
		  $sql="UPDATE plotpayment set pobm='".$_GET['member_id']."' where id=".$_GET['id']."";
               $command = $connection -> createCommand($sql);
               $command -> execute();
			      $this->redirect(Yii::app()->baseUrl."/index.php/memberplot/charges_list?id=".$_REQUEST['plot_id']);
