...
{
//////////START:FP Verify //////////////////
public function actionFpverify(){
	 $command = "C:\Users\PERSONAL\Desktop\Finger Print\bin\Debug\Attendance.exe";
exec($command);
	}
	...
	//////////////for Query Detail
...

	 {
		...
		//	$sql_details  = "SELECT * from query where id='".$_REQUEST['id']."'";
			...
			echo $sql_details  = "select q.*,m.name,mp.plotno,p.plot_detail_address,p.com_res,s.street,sec.sector_name,pro.project_name,p.atype from query q
			left join members m on q.user_id=m.id
			left join memberplot mp on mp.member_id=m.id
			left join plots p on mp.plot_id=p.id
			Left JOIN sectors sec ON p.sector=sec.id
			Left JOIN streets s ON s.id=p.street_id
			Left JOIN projects pro ON pro.id=p.project_id
			Left JOIN size_cat sc ON sc.id=p.size2
			  where q.id='".$_REQUEST['id']."'";exit;
			$result_details = $connection->createCommand($sql_details)->query();
 ...
 $co = $connection->createCommand($sql_memberas)->query();
	...
	if ($result_members!=''){
	 ...
	 foreach($result_members as $key){
			...
			else{?> <b style="color:red;"><a href="Fpverify"><input type="button" value="Not Verified" ></a>
...
public function actionSubmitstatus()
...

	{
	    ...
	    if(isset($_POST['username']) && empty($_POST['username']))
			...
			if(empty($error))
			{
				 /// $password = crypt($_POST['password'], 'rdlpk');
				//  exit;
				  $username = $_POST['username'];
   				 $password = $_POST['password'];
				 ...
				 /// $password_e =base64_encode($password); 
				///  $password_e = str_replace('czI1NXM=', '', $password_e);
				  //////////////////////////////////////////////////////////////////
