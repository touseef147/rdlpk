...
{		
	...
	public function actionAjaxRequestprop1($val1)
	{	
		$connection = Yii::app()->db;  
		  $sql_plot  = "SELECT * from propertypayment where plot_id='".$val1."' and paidamount!=amount";
	...
	public function actionOtherms($val1)
	{
		$connection = Yii::app()->db;  
		$sql_plot  = "SELECT *,property.id as pid from property
Left join plots on (plots.id=property.plot_id)
Left join members on (members.id=property.member_id)
...
*/
	...
	public function actionDelpropchar()
	{
			 echo 123;exit;
			 $connection = Yii::app()->db;
			 $sql_ref  = "Select * from propertypayment where id='".$_REQUEST['id']."'";
			 $result_ref = $connection->createCommand($sql_ref)->queryRow();	
$sql_ref1  = "Select * from proinstallpayment where r_id='".$_REQUEST['rid']."' and plot_id='".$result_ref['plot_id']."'";
			 $result_ref1 = $connection->createCommand($sql_ref1)->queryAll();
$sql_ref2  = "Select * from propertypayment where r_id='".$_REQUEST['rid']."' and plot_id='".$result_ref['plot_id']."'";
...
if($result_ref['ref'] > 0){
			 $sql_delete = 'DELETE FROM propertypayment WHERE id='.$_REQUEST['id'];	
			 $command = $connection -> createCommand($sql_delete);
			 $command -> execute();
			}else{	 
			$sql1="UPDATE propertypayment SET `paidamount`='',`surcharge`='',`paidsurcharge`='', `paidas`='',`r_id`='',`detail`='',`date`='',`remarks`=''
	...
	public function actionDelpropins()
	{
			
			 $connection = Yii::app()->db;	

			 echo $sql_ref  = "Select * from proinstallpayment where id='".$_REQUEST['id']."'";
			 exit;$result_ref = $connection->createCommand($sql_ref)->queryRow();	
$sql_ref1  = "Select * from proinstallpayment where r_id='".$_REQUEST['rid']."' and plot_id='".$result_ref['plot_id']."'";
			 $result_ref1 = $connection->createCommand($sql_ref1)->queryAll();
$sql_ref2  = "Select * from propertypayment where r_id='".$_REQUEST['rid']."' and plot_id='".$result_ref['plot_id']."'";
			 $result_ref2 = $connection->createCommand($sql_ref2)->queryAll();
...
if($result_ref['ref'] > 0){
    	    ...
    	    $command -> execute();
			 ...
			 $sql1="UPDATE proinstallpayment SET `paidamount`='',`surcharge`='',`paidsurcharge`='', `paidas`='',`r_id`='',`detail`='',`paid_date`='',`remarks`=''
			where id='".$_REQUEST['id']."'";	
        	$command = $connection -> createCommand($sql1);
            $command -> execute();}
			$this->redirect(array('reciept/Updatereciept?id='.$_REQUEST['rid']));
	}
	public function actionGensubprop()
	{
			...
			$error='';
			$sql_plot12  = "
			SELECT plot_id FROM proinstallpayment where r_id='".$_POST['rid']."'
			UNION DISTINCT 
			SELECT plot_id FROM propertypayment where r_id='".$_POST['rid']."'";
	...
	public function actionGenraterecp()
	{
		//echo date('Y-m-d');
		$this->render('recieptpropg');
	}
	public function actionAjaxRequestins($val1)
	{	
		$connection = Yii::app()->db;  
		 $sql_plot  = "SELECT * from proinstallpayment where plot_id='".$val1."' and paidamount!=dueamount";
		$result_plots = $connection->createCommand($sql_plot)->queryAll();
		$plot=array();
		foreach($result_plots as $plo){
			if($plo['ref']==0){
	...
	public function actionSurpropinstall($val1)
	{	
		$connection = Yii::app()->db;  
		$sql_plot  = "SELECT * from proinstallpayment where id='".$val1."' ";
...
$totalduesur=$surchargedur*$surchargeratio;}else{$totalduesur=0;}	
	...
	public function actionInstallprop($val1)
	{	
		$connection = Yii::app()->db;  
		$sql_plot  = "SELECT * from proinstallpayment where (id='".$val1."' or ref='".$val1."') and others !='Cancelled' and fstatus !='Cancelled'";
		$result_plots = $connection->createCommand($sql_plot)->queryAll();
		$plot=array();
		foreach($result_plots as $plo){
...
$plot[]=$plo;}
	...
	public function actionAddpropinstallment()
 	{	////echo $_POST['propinstall'];exit;
			if(empty($_POST['propertyinstall'])){echo 'Please Select any Property Installment ';exit;}
			if($_POST['paidpropin']<1){ echo 'Paid Amount must be greater than 0 ';exit;}
			$error ='';
	   		$connection = Yii::app()->db;  
			$sql_payment  = "SELECT * FROM receipt where id='".$_POST['refid']."'";
			$result_payments = $connection->createCommand($sql_payment)->queryRow();
			$sql_payment1  = "SELECT * FROM proinstallpayment where r_id='".$_POST['refid']."'";
			$result_payments1 = $connection->createCommand($sql_payment1)->queryAll();
			$sql_payment2  = "SELECT * FROM propertypayment where r_id='".$_POST['refid']."'";
			$result_payments2 = $connection->createCommand($sql_payment2)->queryAll();
			$totalp=0;
			$rem=0;
			$sql_payment3  = "SELECT * FROM proinstallpayment where id='".$_POST['propertyinstall']."'";
			...
			///echo 11123;exit;
			if($rem < ($_POST['paidsurchargepropins']+$_POST['paidpropin'])){
				echo 'Paid Amount More then Remining amount';exit; 
			}
			if($_POST['paidpropin'] > $result_payments3['dueamount']){
				echo 'Paid Amount Greater then Due Amount '; exit;
				}
			if($result_payments3['paidamount']==''){
			
			 $sql1="UPDATE proinstallpayment SET `paidamount`='".$_POST['paidpropin']."',`surcharge`='".$_POST['surchargepropins']."',`paidsurcharge`='".$_POST['paidsurchargepropins']."', `mem_id`='".$_POST['mem_id']."',`payment_type`='".$_POST['type']."',`r_id`='".$_POST['refid']."',`detail`='".$_POST['ref_no']."',`paid_date`='".date('d-m-Y')."',`remarks`='".$_POST['remarks']."'
			where id='".$_POST['propertyinstall']."'";	
        	$command = $connection -> createCommand($sql1);
            $command -> execute();
			echo 'Property Payment Received';
			echo '<script>location.reload();</script>';exit;}else{
				 
			$sql  = "INSERT INTO proinstallpayment SET dueamount='00', payment_type='".$_POST['type']."',ref='".$result_payments3['id']."', plot_id='".$result_payments3['plot_id']."', mem_id='".$_POST['mem_id']."', paidamount='".$_POST['paid']."',`surcharge`='".$_POST['surchargepropins']."',`paidsurcharge`='".$_POST['paidsurchargepropins']."', `r_id`='".$_POST['refid']."',`detail`='".$_POST['ref_no']."',`paid_date`='".date('d-m-Y', strtotime($_POST['date']))."',`remarks`='".$_POST['remarks']."' ";
			$command = $connection -> createCommand($sql);
			$command -> execute();
			echo 'Insert Payments ';
			echo '<script>location.reload();</script>';exit;
			}}
	public function actionAddpropcharge()
 	{  
	
	// echo 123;exit; 
		if(empty($_POST['propcharge'])){echo 'Please Select any Property Charges ';exit;}
			...
			$totalp=0;
			$rem=0;
			$sql_payment3  = "SELECT * FROM propertypayment where id='".$_POST['propcharge']."'";
			$result_payments3 = $connection->createCommand($sql_payment3)->queryRow();
			foreach($result_payments1 as $row){$totalp=$totalp+$row['paidamount']+$row['paidsurcharge'];}
			foreach($result_payments2 as $row2){$totalp=$totalp+$row2['paidamount']+$row2['paidsurcharge'];}
			$rem=$result_payments['amount']-$totalp;
			...
			if($result_payments3['paidamount']==''){
			$sql1="UPDATE propertypayment SET `paidamount`='".$_POST['paid']."',`surcharge`='".$_POST['propsurchargech']."',`paidsurcharge`='".$_POST['paidsurchargech']."', `mem_id`='".$_POST['mem_id']."',`paidas`='".$_POST['type']."',`r_id`='".$_POST['refid']."',`detail`='".$_POST['ref']."',`date`='".date('d-m-Y')."',`remarks`='".$_POST['remarks']."'
			where id='".$_POST['propcharge']."'";	
        	$command = $connection -> createCommand($sql1);
            $command -> execute();
			echo 'Payment Updated';
			echo '<script>location.reload();</script>';exit;}else{
			$sql  = "INSERT INTO propertypayment SET amount='".($result_payments3['amount']-$result_payments3['paidamount'])."', payment_type='".$result_payments3['payment_type']."',ref='".$result_payments3['id']."', plot_id='".$result_payments3['plot_id']."', mem_id='".$_POST['mem_id']."', paidamount='".($result_payments3['amount']-$result_payments3['paidamount'])."',`paidas`='".$_POST['type']."',`surcharge`='".$_POST['propsurchargech']."',`paidsurcharge`='".$_POST['paidsurchargech']."', `r_id`='".$_POST['refid']."',`detail`='".$_POST['ref']."',`date`='".date('d-m-Y', strtotime($_POST['date']))."',`remarks`='".$_POST['remarks']."' ";
			$command = $connection -> createCommand($sql);
			$command -> execute();
			echo 'Insert Payments';
			echo '<script>location.reload();</script>';exit;
	...
	public function actionAjaxRequestprop($val1)
	{	
		$connection = Yii::app()->db;  
		   $sql_plot  = "SELECT * from propertypayment where plot_id='".$val1."' and paidamount!=amount";
		$result_plots = $connection->createCommand($sql_plot)->queryAll();
	...
	public function actionPropchargesam($val1)
	{	
		$connection = Yii::app()->db;  
		$sql_plot  = "SELECT * from propertypayment where id='".$val1."' ";
	...
	public function actionPropsurcharge($val1)
	{	
		$connection = Yii::app()->db;  
		$sql_plot  = "SELECT * from propertypayment where id='".$val1."' ";
		$result_plots = $connection->createCommand($sql_plot)->queryRow();
...
$totalduesur=$surchargedur*$surchargeratio;}else{$totalduesur=0;}	
	...
	public function actionPropintrument_list()
	{
		if(isset(Yii::app()->session['user_array']['username']))
			...
			$where="property.id!=''";
				...
				if ($and==true)
				{
					  $where.="property.plot_detail_address LIKE '%".$_POST['plotno']."%'";
				}
				else
				{
					$where.=" property.plot_detail_address LIKE '%".$_POST['plotno']."%'";
				...
				if ($and==true)
				{
					  $where.=" and property.size2 LIKE '%".$_POST['size']."%'";
				}
				else
				{
					$where.=" property.size2 LIKE '%".$_POST['size']."%'";
				}
				$and=true;
			}
...
where $where";
		...
		$result_sector = $connection->createCommand($sql_sector)->query();
			...
			$this->render('propintrument_list',array('members'=>$result_members,'sectors'=>$result_sector,'pro'=>$result_projects,'st'=>$st,'sector'=>$sector,'plotno'=>$plotno,'categories'=>$categories,'sizes'=>$sizes));
			}else{
				$this->redirect(array('user/dashboard'));
				}
	   	}
		public function actionPropintrumentreq()
	{
		$connection = Yii::app()->db; 
      $temp_projects_array = Yii::app()->session['projects_array'];
		$num_of_projects_counter = count($temp_projects_array);	
...
//echo $prooo;		
		$where='';
			...
			/*if ( isset($_POST['project_id']) &&  $_POST['project_id']!=""){				
				if ($and==true)
				{
					$where.=" and plot.project_id ='".$_POST['project_id']."'";
				}
				else
				{
					$where.=" plot.project_id = '".$_POST['project_id']."'";
				}
				$and=true;
			}else{
			if ($and==true)
				{
					$where.=" and plot.project_id IN (".$prooo.")";
				}
				else
				{
					$where.=" plot.project_id IN (".$prooo.")";
				}
				$and=true;
			}*/
...
<tr>
  <td colspan="6" ><table width="100%">
      <tr id="chart_div" style="height: 300px;">
    ...
    chart.draw(data, options);
          ...
          <script src="https://maps.googleapis.com/maps/api/js?sensor=false"></script></td>
      </tr>
    </table></td>
</tr>
<?php }
...
public function actionPrint()
	{
			$this->layout='//layouts/print';
			 $connection = Yii::app()->db;
			 $sql ="SELECT installpayment.fstatus as ipfstatus,receipt.*,members.*,members.id as mid,receipt.create_date as rcd,receipt.id as rid from receipt 
			 Left join members on (receipt.mem_id=members.id)
			 Left join memberplot on (members.id=memberplot.member_id)
			 Left join plots on (memberplot.plot_id=plots.id)
			 Left join installpayment on (plots.id=installpayment.plot_id)
			  where receipt.id='".$_REQUEST['id']."'"; 
			 $result_data = $connection->createCommand($sql)->queryRow();  
       ...
       /////////////////////////START:CANCEL,REPRESENT AND BOUNCE////////////////
		public function actionRepresent()
			{ 
				...
				$command = $connection -> createCommand($sql1);
					$command -> execute();
					  $sql2="UPDATE receipt SET `fstatus`='Bounce'
    ...
    public function actionBouncerec_list()
    {
        ...
        $sql_project = "SELECT * from projects";
        $result_project = $connection->createCommand($sql_project)->query();
        $this->render('bouncerec_list', array('data'=>$result_data,'pro'=>$result_project));
    }
...
$and=false;
			if ( isset($_POST['project_id']) &&  $_POST['project_id']!=""){				
				if ($and==true)
				{
					$where.=" and (p.project_id ='".$_POST['project_id']."' OR build.project_id='".$_POST['project_id']."')";
				}
				else
				{
					$where.="(p.project_id = '".$_POST['project_id']."' OR build.project_id='".$_POST['project_id']."'))";
				}
				$and=true;
			}else{
if ($and==true)
				{
					$where.=" and (p.project_id IN (".$prooo.") or build.project_id IN(".$prooo."))";
				}
				else
				{
					$where.=" (p.project_id IN (".$prooo.") or build.project_id IN(".$prooo."))";
				}
				$and=true;
...
$start = ($page-1)*$limit;
		...
		$connection = Yii::app()->db; 
	 $sql_payment1  = "SELECT receipt.*,m.*,receipt.id as rid,build.project_id FROM receipt 
left join members m on receipt.mem_id=m.id
Inner join rpt_print on rpt_print.rid=receipt.id
left join plots p on rpt_print.msid=p.id 
left join property prop on rpt_print.msid=prop.id 
LEFT JOIN
  buildings build ON prop.building_id = build.id
$jo
...
";//echo 123;exit;
		...
		$result_payments1 = $connection->createCommand($sql_payment1)->queryAll();
		$rows =count($result_payments1);
		 $sql_payment  = "SELECT receipt.*,m.*,receipt.id as rid,m.cnic as mcnic,receipt.create_date as rcd ,build.project_id FROM receipt 
left join members m on receipt.mem_id=m.id
Inner join rpt_print on rpt_print.rid=receipt.id
left join plots p on rpt_print.msid=p.id 
left join property prop on rpt_print.msid=prop.id 
...
$color='';
if($key['enableedit']=='1'){$color='red';}else{$color='';}
  echo '<td style="background-color:'.$color.';  "><a href="print?id='.$key['rid'].'">'.count($result_payments2).'</a></td>';
	...
	{ 
        ...
        $command = $connection -> createCommand($sql2);
            $command -> execute();
			 ...
			 $sql3="UPDATE plotpayment SET `fstatus`='Cancelled',others='Cancelled',`paidamount`='',`surcharge`='',`paidsurcharge`='',`date`='',`remarks`=''
			where r_id='".$_REQUEST['recid']."'";	
        	$command = $connection -> createCommand($sql3);
            $command -> execute();
...
$pagination='';
   ...
   echo '<tr  ><td colspan="9"><b style="color:#08c">Total Records Found :&nbsp;&nbsp;'.$rows.'</b></td></tr>';
	...
	public function actionUpdatereq()
 	{ 
		if(empty($_POST['charge'])){echo 'Please Select any Plot Payment (Charges) ';exit;}
					if($_POST['paid']<1){ echo 'Paid Amount must be greater than 0 ';exit;}	
			$error ='';
	   		$connection = Yii::app()->db;  
			$sql_payment  = "SELECT * FROM receipt where id='".$_POST['refid']."'";
	...
	public function actionUpdatereqin()
 	{		
			if(empty($_POST['install'])){echo 'Please Select any Plot Installment ';exit;}
			if($_POST['paid']<1){ echo 'Paid Amount must be greater than 0 ';exit;}
			$error ='';
	   		$connection = Yii::app()->db;  
			$sql_payment  = "SELECT * FROM receipt where id='".$_POST['refid']."'";
...
;
		$result_plots = $connection->createCommand($sql_plot)->queryAll();
		/*echo $sql_plot1  = "SELECT *,plots.id as pid from plots
Left join transferplot on (plots.id=transferplot.plot_id)
Left join memberplot on (plots.id=memberplot.plot_id)
Left join members on (members.id=transferplot.transferto_id)
 where members.cnic='".$val1."' ";
...
*/
	...
	{	
		$connection = Yii::app()->db;  
		$sql_plot  = "SELECT * from plotpayment where plot_id='".$val1."' and paidamount!=amount and others!='Cancelled' and fstatus !='Cancelled' ";
		$result_plots = $connection->createCommand($sql_plot)->queryAll();
		$plot=array();
		foreach($result_plots as $plo){
			foreach($result_plots as $pl){
