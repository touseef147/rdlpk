  ...
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
	...
	public function actionLocationch()
	{
		$error='';
						...
						if((isset($_POST['PLcharges']) && empty($_POST['PLcharges'])))
						{
							$error .= 'Please Enter Location Charges<br />';
						}
					...
					$command -> execute();	
						...
						echo 'Location Charges Applied';exit; 
				}else{
					echo $error;
					}
	}
	...
	///////////////////////START: PROPERTY MODULE///////////////////////
		public function actionAjaxRequest1($building_id,$size,$floor_id)
		{	

		$connection = Yii::app()->db;  
//if($_POST['pptype']=='C'){$type='Commercial';}else{$type='Residential';}
		 $sql_plot  = "SELECT * from property where building_id='".$building_id."' and size2='".$size."' and floor_id='".$floor_id."' and status=''";
...
public function actionSearchreqprop()
		...
		if(Yii::app()->session['user_array']['per3']=='1')
			{
		$where="mp.status='Sales'";
		$and=true;
		 if (isset($_POST['sector']) && $_POST['sector']!=""){
				$where.="p.sector LIKE '%".$_POST['sector']."%'";
				$and = true;
				$sector=$_POST['sector'];

		 }
			 if (isset($_POST['com_res']) && $_POST['com_res']!=""){
				$where.="p.com_res LIKE '%".$_POST['com_res']."%'";
				$and = true;
			...
			if (isset($_POST['plotno']) && $_POST['plotno']!=""){
				...
				if ($and==true)
				{
					  $where.=" and mp.plot_detail_address ='".$_POST['plotno']."'";
				}
				else
				{
					$where.=" mp.plot_detail_address ='".$_POST['plotno']."'";
				}
				$and=true;
			}
			if (isset($_POST['size']) && $_POST['size']!=""){
				...
				if ($and==true)
				{
					  $where.=" and p.size2 LIKE '%".$_POST['size']."%'";
				}
				else
				{
					$where.=" p.size2 LIKE '%".$_POST['size']."%'";
				}
				$and=true;
			}
			$catt='';
...
$start = ($page-1)*$limit;
    ...
    $sql_memberas = "SELECT mp.member_id,mp.plotno,mp.create_date,p.id,p.type,mp.app_no,p.project_id,m.name,mp.plotno,m.image,m.sodowo,m.cnic,p.plot_detail_address,mp.plot_id,mp.status,p.plot_size,p.project_id,p.street_id,s.street,s.id,j.id,j.project_name FROM property mp left join members m on mp.member_id=m.id left join plots p on mp.plot_id=p.id left join streets s on p.street_id=s.id left join projects j on p.project_id=j.id 
	left join size_cat_prop scp on mp.size2=scp.id
	 where $where"; 
        $co = $connection->createCommand($sql_memberas)->query();
		$rows =count($co);


    echo $sql_member = "SELECT mp.id as pid, scp.size,mp.member_id,mp.plotno,mp.create_date,p.id,p.type,mp.app_no,p.project_id,m.name,mp.plotno,m.image,m.sodowo,m.cnic,p.plot_detail_address,mp.plot_id,mp.status,p.plot_size,p.project_id,p.street_id,s.street,s.id,j.id,j.project_name , floors.name as fname, buildings.name as buildname FROM property mp left join members m on mp.member_id=m.id left join plots p on mp.plot_id=p.id left join streets s on p.street_id=s.id left join projects j on p.project_id=j.id 
	left join size_cat_prop scp on mp.size2=scp.id
	Left JOIN buildings  ON (buildings.plot_id = p.id)
	Left JOIN floors  ON (mp.floor_id = floors.id)
	 where $where and mp.fstatus='Sales' limit $start,$limit"; 
...
$home=Yii::app()->request->baseUrl; 
			$F='';
			$M='';
		 echo '<tr><td>'.$key['plotno'].'</td><td>'.$key['app_no'].'</td><td><img src="/upload_pic/'.$key['image'].'" width="100" height="130" /></td><td><a href="'.$home.'/index.php/user/memhistory?id='.$key['member_id'].'">'.$key['name'].'</a></td><td>'.$key['sodowo'].'</td><td>'.$key['cnic'].'</td><td>'.$key['size'].'&nbsp;('.$key['plot_size'].')</td><td><a href="'.$home.'/index.php/user/plothistory?id='.$key['pid'].'"><strong>'.$key['plot_detail_address'].'</strong></a><td>'.$key['buildname'].'</td><td>'.$key['fname'].'</td><td>'.$key['project_name'].'</td><td>
			<a target="_blank" href="propreq_detail?plot_id='.$key['pid'].'">Request Detail</a></br>
			<a target="_blank" href="'.Yii::app()->request->baseUrl.'/index.php/memberplot/amembers?mid=">Associates Member</a>
  </td>';
...
$pagination='';
 ...
 echo '<tr  ><td colspan="12"><b style="color:#08c">Total Records Found :&nbsp;&nbsp;'.$rows.'</b></td></tr>';
	...
	exit;	 	
		...
		public function actionAllotmentsprop_lis()
		{	
...
//if($and ==true){echo 0;}else{echo 1;}exit;
			...
			if (!empty($_POST['plot_detail_address'])){
...

				{
					...
					$where.=" and mp.plot_detail_address LIKE '%".$_POST['plot_detail_address']."%'";
...

				{
					...
					$where.=" mp.plot_detail_address LIKE '%".$_POST['plot_detail_address']."%'";
	...
	$connection = Yii::app()->db; 
	 ...
	 $sql_member = "SELECT mp.member_id,mp.plotno,mp.create_date,p.id,p.type,p.project_id,m.name,mp.plotno,m.image,m.sodowo,m.cnic,p.plot_detail_address,mp.plot_id,mp.status,p.plot_size,p.project_id,p.street_id,s.street,s.id,j.id,j.project_name FROM property mp
...
echo'  </td></tr>'; 
			...
			$this->render('allotmentsprop_lis',array('members'=>$result_members,'error'=>$error,'projects'=>$result_projects));
	}else{
		 $this->redirect(array("user/dashboard"));	

		}
	}
			public function actionPropreq_detail()
		{
	...
	if(Yii::app()->session['user_array']['per12']=='1')
			...
			$connection = Yii::app()->db; 	
				 ...
				 $sql_details  = "SELECT scp.size,scp.code as scode,mp.member_id,mp.plotno,mp.create_date,p.id,p.type,mp.app_no,p.project_id,m.name,mp.plotno,m.image,m.sodowo,m.cnic,p.plot_detail_address,mp.plot_id,mp.status,p.plot_size,p.project_id,p.street_id,s.street,s.id,j.id,j.project_name,j.code,floors.name as fname, buildings.name as buildname FROM property mp left join members m on mp.member_id=m.id left join plots p on mp.plot_id=p.id left join streets s on p.street_id=s.id left join projects j on p.project_id=j.id 
	left join size_cat_prop scp on mp.size2=scp.id
	Left JOIN buildings  ON (buildings.plot_id = p.id)
	Left JOIN floors  ON (mp.floor_id = floors.id)
	 where mp.fstatus='Sales' and mp.id=".$_REQUEST['plot_id'];
		$result_details = $connection->createCommand($sql_details)->query();
			...
			$this->render('propreq_detail',array('plotdetails'=>$result_details, 'plotpayments'=>$result_payments)); 
...
////////////////////////START: ALLOT a Property///////////////////////////
public function actionPropmemberplot()
		{	
	 if(Yii::app()->session['user_array']['per12']=='1')
			{
		$connection = Yii::app()->db;  
...
";
		$result_plan = $connection->createCommand($sql_plan)->query();	
	     ...
	     $result_building = $connection->createCommand($sql_building)->query();
			...
			$sql_categories  = "SELECT * from categories";
		    $categories = $connection->createCommand($sql_categories)->query();
		$this->render('propmemberplot',array('projects'=>$result_projects,'country'=>$result_country,'plan'=>$result_plan,'categories'=>$categories,'size'=>$result_size,'buildings'=>$result_building,'ptype'=>$result_ptype));
	...
	//	echo 123;exit;
						 ...
						 if(empty($error)){  
						    ...
						    $sqlcharges="INSERT INTO propertypayment SET payment_type='MS Fee',amount='10000', duedate='".$_POST['date']."', plot_id='".$_POST['plot_id']."',mem_id='".$result_data['id']."'";		
					...
					$sql="Update property SET member_id='".$result_data['id']."',
											app_no='".$_POST['appnoo']."',
											...
											status='Sales',
											fstatus='Sales',
											uid='".Yii::app()->session['user_array']['id']."' WHERE id='".$_POST['plot_id']."'";
...
///////////////////////////////////END: ALLOT a Property//////////////////////////////////////	
	public function actionSubmitprop()
	{// echo 123;exit;
		$error='';
		$connection = Yii::app()->db;
		$pid='';
								...
								if ((isset($_POST['tempms']) && empty($_POST['tempms']))){
								 echo $error.="Membership No required. <br>";exit;
									...
									$q ="SELECT * from property where plotno='".$ms."'"; 
									$result_q = $connection->createCommand($q)->queryRow();
									if ($result_q['plotno']==$ms){
									echo $error.="Membership # Already Added Try Another. <br>";
									exit;
									}
									}
									 $sql="Update property SET status='New', fstatus='',plotno='".$ms."' where id='".$pid."'";	
        $command = $connection -> createCommand($sql);
        $command -> execute();
		echo '<script>location.href="allotmentsprop_lis";</script>';exit;
...
///////////////////////////////////END: PROPERTY MODULE	
///////////////////////////////////DEALER TO MEMBER TRANSFER: START///////////////////////////

public function actionDealerrequest()
	 	{
										 $connection = Yii::app()->db;  
									 $base=$_POST['cnic']; 
									  $uid=Yii::app()->session['user_array']['id'];
									...
									if(!empty($error)){
									 ...
									 $sql  = "UPDATE memberplot SET status='Approved' where plot_id=".$_POST['plot_id']."";                      
					    			 $command = $connection -> createCommand($sql);
			            			 $command -> execute();			
									 $transferto_memberid = $result_data['id'];
	            					 $sql="INSERT INTO transferplot SET plot_id='".$_POST['plot_id']."',uid='".$uid."',transferfrom_id='".$_POST['transfer_from_memberid']."',transferto_id='".                                     $transferto_memberid."',status='Sales',fstatus='Sales',cmnt='New Request',create_date='".date('Y-m-d')."' ";	 
        		   					 $command = $connection -> createCommand($sql);
                      				 $command -> execute();
...
public function actionTransferplot_dealer()
		{	if(isset(Yii::app()->session['user_array']['username'])){	
...
left join size_cat sc on p.size2=sc.id 
   WHERE p.id ='".$plotid."' ";
	...
	$this->render('transferplot_dealer',array('plotdetails'=>$plotdetails));
	}
			else{$this->redirect(Yii::app()->baseUrl."/index.php/user/dashboard"); }

	exit();
	}
////////////////////////////////////////////DEALER TO MEMBER TRANSFER: END///////////////////////////////////////////	
...
public function actionSurchargedef()
		{
			...
			if((isset($_POST['plot_id']) && empty($_POST['plot_id']))  ||(isset($_POST['duedate']) && empty($_POST['duedate']))  || (isset($_POST['charges_id']) && empty($_POST['charges_id'])) || (isset($_POST['comment']) && empty($_POST['comment']))|| (isset($_POST['total']) && empty($_POST['total'])))
			{
				...
				if(empty($error)){
						...
						$sql_charges  = "SELECT * from installpayment where plot_id='".$_POST['plot_id']."' and surcharge_re=0 and(paidsurcharge='' or paidsurcharge < 0)";
						$result_charges = $connection->createCommand($sql_charges)->queryAll();
						$totalssss=0;
						foreach($result_charges as $key){
...
if($_POST['allotmentstatus']==2){if ($and==true)
			...
			if (!empty($_POST['appno'])){
				...
				if ($and==true)
				{
					$where.=" and mp.app_no ='".$_POST['appno']."'";
				}
				else
				{
					$where.="mp.app_no ='".$_POST['appno']."'";
				}
				$and==true;
			}
			if (!empty($_POST['plotno'])){
...
$connection = Yii::app()->db; 
echo $sql_memberas = "SELECT * FROM memberplot mp
left join members m on mp.member_id=m.id
left join plots p on mp.plot_id=p.id
				...
				$error='';
							  ...
							  if ((isset($_POST['insplan']) && empty($_POST['insplan']))){
								 ...
								 //}
									...
									//}
									//}
						$q ="SELECT * from memberplot left join plots on plots.id=memberpolot.plot_id where memberplot.app_no='".$_POST['appnoo']."' and plots.project_id='".$_POST['project']."'"; 
									$result_q = $connection->createCommand($q)->queryRow();
									if (!empty($result_q)){
...
FROM
 ...
 $sql_payment  = "SELECT * FROM installpayment where id='".$_GET['id']."'";
	...
	public function actionAjaxRequest7($val1,$sizep,$pro)
		{	
		$connection = Yii::app()->db;  
		 $sql_plot  = "SELECT * from installment_planp where sizep='".$sizep."' and project_id='".$pro."' ";
		$result_plots = $connection->createCommand($sql_plot)->query();
		$plot=array();
		foreach($result_plots as $plo){
			$plot[]=$plo;
		    ...
		    $error ='';
									...
									if(!empty($error)){
	            					   ...
	            					   /*$sql="INSERT INTO transferplot SET plot_id='".$_POST['plot_id']."',uid='".$uid."',transferfrom_id='".$_POST['transfer_from_memberid']."',transferto_id='".                                     $transferto_memberid."',status='Sales',fstatus='Sales',cmnt='New Request',create_date='".date('Y-m-d H:i:s')."' ";	 
        		   					 $command = $connection -> createCommand($sql);
                      				 $command -> execute();*/
									 	 $sql1="UPDATE transferplot SET plot_id='".$_POST['plot_id']."',uid='".$uid."',transferfrom_id='".$_POST['transfer_from_memberid']."',transferto_id='".                                     $transferto_memberid."',status='Sales',fstatus='Sales',cmnt='New Request',create_date='".date('Y-m-d H:i:s')."' WHERE plot_id='".$_POST['plot_id']."' ";	 
        		   					 $command = $connection -> createCommand($sql1);
                      				 $command -> execute();
...
/*
	...
	public function actionAjaxReqsize($val1)
		{	
		$connection = Yii::app()->db;  
		$sql_city  = "SELECT * from size_cat where size='".$val123."' and type=";
...
left join projects j on p.project_id=j.id where p.type='plot' and mp.status='Approved'";
	...
	public function actionSearchreq()
		...
		$sodowo=mysql_real_escape_string($_POST['sodowo']);
		$cnic=mysql_real_escape_string($_POST['cnic']);
		$app_no=mysql_real_escape_string($_POST['app_no']);
	///	$allotmentstatus=mysql_real_escape_string($_POST['allotmentstatus']);
...
$check=1;
    ...
    if($key['mmtype']=='Dealer'){
	 echo'<li role="presentation"><a target="_blank" href="transferplot_dealer?plot_id='.$key['plot_id'].'"> Transfer Plot D</a></li>';	 }
...
$resulttest = $connection->createCommand($sqltest)->query();
	...
	';
			...
			if($test['status']!='Requested(T)' && $key['status']=='Approved') {echo '<li role="presentation"><a target="_blank" href="transferplot?plot_id='.$key['plot_id'].'"> 
			Transfer Plot</a></li>';}
			elseif($key['mmtype']=='Dealer'){echo '<li role="presentation"><a target="_blank" href="transferplot?plot_id='.$key['plot_id'].'"> 
			Transfer Plot</a></li>';}
	...
	///	$allotmentstatus=mysql_real_escape_string($_POST['allotmentstatus']);
		...
		$and = false;
			...
			if (!empty($sodowo)){				
				if ($and==true)
				{
					$where.=" and m.sodowo LIKE '%".$sodowo."%'";

				}
				else
				{

					$where.=" m.sodowo LIKE '%".$sodowo."%'";
			...
			if (!empty($project_name)){
				if($and==true)
				{
					$where.=" AND p.project_id LIKE '%".$project_name."%'";
				}
				else
				{
					$where.="  p.project_id LIKE '%".$project_name."%'";
				}
				$and=true;
			}
			...
			if (!empty($plotno)){
				...
				{
					$where.="mp.plotno LIKE '%".$plotno."%'";
				}
				$and==true;
			}
				$connection = Yii::app()->db; 
   
    $sql_member = "SELECT mp.mmtype,mp.member_id,mp.app_no,mp.mstatus as stst,mp.plotno,tp.status as tstatus,mp.create_date,mp.fstatus,p.id,p.type,p.project_id,m.name,mp.plotno,m.image,m.sodowo,m.cnic,p.plot_detail_address,mp.plot_id,mp.status,p.plot_size,p.status as pstatus,p.project_id,p.street_id,s.street,s.id,j.id,j.project_name,sec.sector_name,size_cat.size FROM memberplot mp
left join members m on mp.member_id=m.id
left join plots p on mp.plot_id=p.id
...
if($key['tstatus']=='New Request' or $key['tstatus']=='Sales'){
	}else {
...
echo '</br><a target="_blank" href="payment_details?id='.$key['plot_id'].'&& pid='.$key['project_id'].'">Payment Details</a> <br/>';
 if($key['mmtype']=='Dealer'){
	 echo'<a target="_blank" href="transferplot?plot_id='.$key['plot_id'].'"> Transfer Plot</a>';
	 }else {echo'';}
...
$land  = "SELECT * FROM installpayment	where plot_id='".$_REQUEST['id']."' ";
		...
		$members = $connection->createCommand($member)->queryRow();
		 ...
		 echo $sql_payment  = "SELECT * FROM plotpayment where plot_id='".$_REQUEST['id']."' and (mem_id='".$members['member_id']."' or payment_type NOT IN ('MS Fee','Transfer Fee'))";
...
echo'  </td></tr>'; 
	...
	public function actionTransferreq()
		...
		$and = false;
			if (!empty($_POST['name1'])){
				$where.=" m_from.name LIKE '%".$_POST['name1']."%'";
				$and = true;
			}
			if (!empty($_POST['sodowo'])){				
				if ($and==true)
