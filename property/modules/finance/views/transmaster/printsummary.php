<?php

use yii\helpers\Html;
use yii\grid\GridView;

/* @var $this yii\web\View */
/* @var $searchModel app\models\application\FmstransmasterSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$this->title = 'Fmstransmasters';
$this->params['breadcrumbs'][] = $this->title;




?>
                                            <table cellpadding="3" cellspacing="0" width="100%">
  <thead>
      <tr class="HeaderBlue" height="25">
      <th  class="HeaderLeft style13" style="border-bottom:thin solid" align="center" width="20">      </th>
      <th class="HeaderMiddle" style="border-bottom:thin solid">Serial no      </th>
      <th class="HeaderMiddle" style="border-bottom:thin solid">Trans type      </th>
      <th class="HeaderMiddle" style="border-bottom:thin solid">Trans date      </th>
      <th class="HeaderMiddle" style="border-bottom:thin solid">Remarks      </th>
      <th class="HeaderMiddle" style="border-bottom:thin solid">Entered by      </th>
      <th class="HeaderMiddle" style="border-bottom:thin solid">Ref no      </th>
      <th class="HeaderRight" style="border-bottom:thin solid">      </th>
      </tr>
  </thead>
  <tbody>
<?php      $rows = $dataProvider->getModels(); ?>
<?php      foreach ($rows as $row) { ?>
      <tr>
      <td class="GridLeftCell">      </td>
      <td class="GridMiddleCell" align="left"><?php echo $row->serial_no; ?>      </td>

                                                                  <td class="GridMiddleCell" align="right"><?php echo $row->trans_type; ?>      </td>
      <td class="GridMiddleCell" align="left"><?php echo $row->trans_date; ?>      </td>
      <td class="GridMiddleCell" align="left"><?php echo $row->remarks; ?>      </td>
      <td class="GridMiddleCell" align="right"><?php echo $row->entered_by; ?>      </td>
      <td class="GridMiddleCell" align="left"><?php echo $row->ref_no; ?>      </td>
      <td class="GridRightCell">       </td>
      </tr>
      <?php } ?>
  </tbody>
</table>
    

