...
class Propertyapplication extends \yii\db\ActiveRecord {
    ...
    public function rules() {
        return [
            ...
            [['project_id', 'sales_center_id', 'member_id', 'property_type', 'property_size', 'dealer_id', 'nominee_id', 'voucher_id', 'property_against', 'installment_plan', 'installment_plan_development', 'application_status', 'receipt_entered', 'owner_id', 'is_reserved_plot'], 'integer'],
            [['application_date', 'plan_start_date', 'submission_date'], 'date'],
            [['application_no'], 'string', 'max' => 50],
            [['submission_remarks'], 'safe'],
//            [['application_no'], 'unique', 'targetAttribute' => ['project_id'],'message'=>'Another form with same details already exist.']
        ];
    }
    ...
    public function getStatustitle() {
        ...
        elseif ($this->application_status == 10)
            return "Forwarded";
//        elseif ($this->application_status == 11)
//            return "Verified";
//        elseif ($this->application_status == 12)
//            return "Rejected";
//        elseif ($this->application_status == 13)
//            return "Hold";
//        elseif ($this->application_status == 20)
//            return "Approved";
        else
            return "";
    }
...
//            20 => "Approved",
    ...
    public function getOwner() {
        return $this->hasOne(Members::className(), ['id' => 'owner_id']);
    }
    ...
    public function register($datapchoices, $reservedplotno = 0) {
        ...
        if ($valid) {
            ...
            $this->is_reserved_plot = ($reservedplotno == 0 ? 0 : 1);
        ...
        if ($valid && $this->property_against == 1) {
        ...
        if ($valid) {
            $valid = Plots::updatePlotReservation($this,$reservedplotno) && $valid;
        }
    ...
    public function edit(&$datapchoices) {
        if ($this->validaterecord() == FALSE) {
            return FALSE;
        }
...
//        }
        if ($this->application_id != 0) {
            $temp = Propertyapplication::find()->where(['application_id' => $this->application_id])->one();
            $file = Plots::find()->where(['application_id' => $this->application_id])->one();
            $plan = Installpayment::find()->where(['plot_id' => $file->id])->all();

            $changeplan = false;

            if ($file->size2 != $this->property_size) {
                $changeplan = true;
            }

            if ($this->plan_start_date != $temp->plan_start_date) {
                $changeplan = true;
            }
            ...
            if ($changeplan) {
                foreach ($plan as $row) {
                    if (floatval($row->paidamount) != 0) {
                        $this->addError('application_no', 'Can not update form because some financial transaction have been made.');
                        return false;
                    }
                }

                if ($valid) {
                    $valid = Plots::attachFilewithApplication($this, TRUE) && $valid;
                }

                if ($valid && $this->installment_plan != 0) {
                    $valid = Installpayment::generatePlanFromApplication($this, true) && $valid;
                }
...
//        }
    ...
    public function cancel() {
        ...
        if ($valid) {
            $valid = Propertyapplicationcomments::updatesystemcomments("Application Cancelled.", 1, $this->application_id) && $valid;
        }
    ...
    public static function updateReceiptstatus($appid, $status) {
        $application = Propertyapplication::find()->where(['application_id' => $appid])->one();

        if ($application != NULL) {
            $application->receipt_entered = $status;
