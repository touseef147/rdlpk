    ...
    public function actionCreate()
    {
        $model = new Projects();
         if ($model->load(Yii::$app->request->post())) {
            $model->pimage = UploadedFile::getInstance($model, 'pimage');     
            if($model->updaterecord())
            {
                ...
                return [
                    "saved",
                    Yii::$app->urlManager->baseUrl . "/index.php?r=property/config/projects/index",
                    "Record Saved",
                    "Record saved successfully"
                ];
            }
			
			 $model->pmap = UploadedFile::getInstance($model, 'pmap');     
            if($model->updaterecord())
            {
                \Yii::$app->response->format = 'json';
			 ...
			 $model->lmap = UploadedFile::getInstance($model, 'lmap');     
            if($model->updaterecord())
            {
                \Yii::$app->response->format = 'json';
