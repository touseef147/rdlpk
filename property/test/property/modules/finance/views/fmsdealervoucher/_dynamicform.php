...
$modeldd=app\models\application\Propertyapplication::find()->where(['dealer_id' => $memberid])->andFilterWhere(['or',['=','receipt_entered','0'],['=','application_id',$appid]])->orderby('application_no ASC')->all();
        ...
        <div class="widget-toolbar no-border">
            ...
            if($appid == NULL || $mode== "entry"){
                ...
                echo Html::dropDownList('Fmsvoucherplotdetail[' . $id2 . '][application_id]', $appid, ArrayHelper::map($modeldd, 'application_id', 'application_no'), // Flat array ('id'=>'label')
                    ...
                    'onchange' => ' 
                                $.get( "' . Url::toRoute('/finance/fmsdealervoucher/dynamicformcontent') . '", { id: $(this).val(), index: ' . $id . ', mode: "' . $mode . '" } )
                                    .done(function( data ) {
    ...
    <div class="widget-body">
        <div id="formcontent<?php echo $id2; ?>" class="widget-main padding-12">
            ...
            if (isset($receipt) && $receipt != NULL) {
                ...
                'idx' => $id2,
                'id' => $id,
                'model' => app\models\application\Propertyapplication::find()->where(['application_id'=>$receipt->application_id])->one(),
                'receipt' => $receipt,
