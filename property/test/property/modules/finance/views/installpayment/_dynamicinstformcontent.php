...
<?= Html::hiddenInput('Fmsvoucherplotdetail[' . $idx . '][transaction_source]', 2) ?>
...
<table style="width: 100%;">
    <tr>
        ...
        <td style="width:150px; <?php echo ($mode == "entry" ? "" : "border-bottom: dotted" );  ?>">
            <?php
            $tempvalid = (isset($receipt) && $receipt != NULL ? $receipt->voucher_plot_detail_id : NULL);
            $tempval = (isset($receipt) && $receipt != NULL ? $receipt->receipt_no : NULL);
            
            echo Html::hiddenInput('Fmsvoucherplotdetail[' . $idx . '][voucher_plot_detail_id]', $tempvalid);
            
            if($mode == "entry"){
                echo Html::textInput('Fmsvoucherplotdetail[' . $idx . '][receipt_no]', $tempval,['style'=> 'background-color:rgb(255, 255, 215);']);
            }else{
                echo $tempval;
            }
            echo (isset($receipt) && $receipt != NULL ? Html::error($receipt, 'receipt_no', ['class' => 'help-block']) : NULL); 
        ...
        <td colspan="5" style="<?php echo ($mode == "entry" ? "" : "border-bottom: dotted" );  ?>">
            ...
            if($mode == "entry"){
                echo Html::textInput('Fmsvoucherplotdetail[' . $idx . '][narration]', $tempval,['style'=> 'background-color:rgb(255, 255, 215);']);
            } else{
                echo $tempval;
            }
    ...
    <tr>
        ...
        <td style="border-bottom: dotted"><?= (!isset($model->ms_no) ? "" : $model->ms_no) ?></td>
        <td style="text-align: right; width: 100px; padding-right: 5px; height: 25px;">Date</td>
        <td style="border-bottom: dotted; width: 100px;"><?= ""//date("d-m-Y", strtotime($model->application_date)) ?></td>
        <td style="text-align: right; width: 100px; padding-right: 5px; height: 25px;">Size</td>
        <td style="border-bottom: dotted; width: 100px;"><?= (!isset($model->plot->sizeCat->size) ? "" : $model->plot->sizeCat->size) ?></td>
        <td style="text-align: right; width: 100px; padding-right: 5px; height: 25px;">Type</td>
        <td style="border-bottom: dotted;  width: 100px;"><?= ""//$model->plot->propertytypetitle ?></td>
    </tr>
    <tr>
        <td style="text-align: right; width: 100px; padding-right: 5px; height: 25px;">Project</td>
        <td colspan="3" style="border-bottom: dotted"><?= (!isset($model->plot->project->project_name) ? "" : $model->plot->project->project_name) ?></td>
...
if($mode=="entry"){
    ?>
        <div class="widget-toolbar">
            <a class="white add_dynamic_row" data-url="<?php echo Yii::$app->urlManager->baseUrl; ?>/index.php?r=finance/fmsvoucher/dynamicinstallpaymentrow&mode=<?php echo $mode; ?>" data-tag="<?php echo $model->ms_id; ?>" data-source="2" data-rowcount="drinstpaymentcount" data-gtotalidx="<?php echo $idx; ?>" data-desttable="instformno<?php echo $idx; ?>" href="#">Add</a>
        </div>
        <?php
}
?>
    ...
    <div class="widget-body">
        <div class="widget-main no-padding">
            <table id="instformno<?php echo $idx; ?>" class="table  table-bordered table-hover">
                <thead>
                    <tr>
