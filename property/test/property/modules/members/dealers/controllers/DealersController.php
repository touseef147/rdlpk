...
/**
 * DealersController implements the CRUD actions for Members model.
 */
...
//            $model->file = UploadedFile::getInstance($model, 'file');
            ...
            if ($model->updaterecord(FALSE)) {
                ...
                return [
                    "saved",
                    Yii::$app->urlManager->baseUrl . "/index.php?r=members/dealers/dealers/update&id=" . $model->parent_id,
                    "Record Saved",
                    "Record saved successfully"
                ];
            }
...
//            return "Failed to remove record";
    ...
    public function actionSidebarsummary() {
        $res = \app\models\Model::checksavedinfo(["r" => "members/dealers/dealers/index"]);
        $myrights = \app\models\Model::getRights("index", "dealers", "members/dealers");
    ...
    protected function findModel($id) {
        if (($model = Members::findOne(['id' => $id, 'is_dealer' => 1])) !== null) {
            return $model;
        } else {
    ...
    public function actionShowrecord($id = 0, $myoptions = null) {
        $model = Members::find()->where(['id' => $id, 'is_dealer' => 1])->one();
    ...
    public function actionCount() {
        $model = \app\models\application\Members::find()->where(['is_dealer' => 1])->select('id')->all();
