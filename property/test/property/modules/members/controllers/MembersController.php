...
class MembersController extends Controller {
    ...
    public function beforeAction($action) {
        ...
        if ($action->actionMethod == "actionChangepassword" || $action->actionMethod == "actionCompleteregistration") {
            if (Yii::$app->member->id == null) {
                $this->redirect("index.php?r=members/portal/default/index");
            } else {
                return parent::beforeAction($action);
            }
        } else if ($action->actionMethod == "actionRegister" || $action->actionMethod == "actionActivatemember") {
            return parent::beforeAction($action);
        } else {
...
//            $model->file = UploadedFile::getInstance($model, 'file');
    ...
    public function actionRegister() {
        ...
        if ($model->load(Yii::$app->request->post())) {
            //$model->picture = UploadedFile::getInstance($model, 'picture');
            $model->acc_activation_key = rand(1000000,9999999);
            $model->is_dealer = 0;
            $model->status = 1;
//            $model->file = UploadedFile::getInstance($model, 'file');
            ...
            if ($model->updaterecord(FALSE)) {
                ...
                return [
                    "saved",
                    Yii::$app->urlManager->baseUrl . "/index.php?r=members/members/activatemember&activationkey=".$model->id,
                    "Record Saved",
                    "Record saved successfully"
                ];
            }
        ...
        $request = Yii::$app->request;
        if ($request->isAjax) {
            return $this->renderPartial('_register', [
                        'model' => $model,
            ]);
        } else {
    ...
    public function actionActivatemember($activationkey) {
        $this->layout = "@app/views/layouts/memarea";

        $model = $this->findModel($activationkey);
        if ($model->load(Yii::$app->request->post())) {
            //$model->picture = UploadedFile::getInstance($model, 'picture');
            $model->is_dealer = 0;
            $model->status = 1;
//            $model->file = UploadedFile::getInstance($model, 'file');
            ...
            if ($model->updaterecord(FALSE)) {
                ...
                return [
                    "saved",
                    Yii::$app->urlManager->baseUrl . "/index.php?r=members/members/completeregistration",
                    "Record Saved",
                    "Record saved successfully"
                ];
            }
        ...
        $request = Yii::$app->request;
        if ($request->isAjax) {
            return $this->renderPartial('_activatemember', [
                        'model' => $model,
            ]);
        } else {
            return $this->render('activatemember', [
                        'model' => $model,
            ]);
        }
    }
...
//            $model->file = UploadedFile::getInstance($model, 'file');
            ...
            if ($model->updaterecord(FALSE)) {
                ...
                return [
                    "saved",
                    Yii::$app->urlManager->baseUrl . "/index.php?r=property/application/propertyapplication/newbooking/",
                    "Record Saved",
                    "Record saved successfully"
                ];
            }
        ...
        $request = Yii::$app->request;
        if ($request->isAjax) {
            return $this->renderPartial('_completeregistration', [
                        'model' => $model,
            ]);
        } else {
...
//            $model->file = UploadedFile::getInstance($model, 'file');
    ...
    public function actionChangepassword() {
        ...
        $model = $this->findModel(Yii::$app->member->identity->id);

        if ($model->load(Yii::$app->request->post())) {
            $data = Yii::$app->request->post();

            if ($data["Members"]["password"] != "") {
                $model->password = $data["Members"]["password"];
            }
            ...
            if ($model->updatepwd()) {
                ...
                return [
                    "saved",
                    Yii::$app->urlManager->baseUrl . "/index.php?r=members/portal/dashboard/index",
                    "Record Saved",
                    "Record saved successfully"
                ];
            }
...
//            $model->sodowo = $parent->name;
    ...
    public function actionCount() {
        ...
        $cmd = $connection->createCommand("select count(id) as noofrecords from members where IFNULL(parent_id,0) = 0");    //IFNULL(is_dealer,0) = 0 and 
        $records = $cmd->queryAll();

        foreach ($records as $rec) {
            return $rec["noofrecords"];
        }
        return 0;
    }
