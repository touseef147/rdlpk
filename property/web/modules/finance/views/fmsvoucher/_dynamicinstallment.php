...
$modeldd = app\models\application\Propertymemberships::find()->where(['member_id' => $memberid])->andFilterWhere(['or', ['=', 'ms_id', $msid]])->all();
    ...
    <div class="widget-header">
        <h4 class="widget-title lighter">
            ...
            if ($msid == NULL || $mode == "entry") {
                ?>
                <input type="checkbox" id="Fmsvoucherplotdetail_<?php echo $id2; ?>_is_selected" name="Fmsvoucherplotdetail[<?php echo $id2; ?>][is_selected]" checked="checked" value="1" onclick="if (this.checked)
                            this.value = 1;
                        else
                            this.value = 0;">&nbsp;
                <?php
        ...
        <div class="widget-toolbar no-border">
            ...
            if ($msid == NULL || $mode == "entry") {
                echo "Membership No.";
                echo Html::dropDownList('Propertymemberships[' . $id2 . '][ms_id]', $msid, ArrayHelper::map($modeldd, 'ms_id', 'ms_no'), // Flat array ('id'=>'label')
                        ['prompt' => '---',
                    'value' => (isset($receipt) && $receipt != NULL ? $receipt->membership_id : ""),
                    'onchange' => ' 
                                $.get( "' . Url::toRoute('/finance/fmsvoucher/dynamicinstformcontent') . '", { id: $(this).val(), index: ' . $id . ', mode: "' . $mode . '" } )
                                    .done(function( data ) {
