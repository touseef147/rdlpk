...
class User extends \yii\db\ActiveRecord {
    ...
    public function updatePicture() {
        if ($this->id != 0 && $this->pic != "" && $this->pic != null) {
            (file_exists('uploads/users/' . $this->pic) ? unlink('uploads/users/' . $this->pic) : "");
        }
        if ($this->uimage != null) {
            $this->uimage->saveAs('uploads/users/' . $this->id . '.' . $this->uimage->extension);
            $this->pic = $this->id . "." . $this->uimage->extension;
            $this->save(FALSE);
        }
...
//        $passwordcheck = \app\models\application\User::find()->where(['password' => $this->password]);
//
//        if ($this->id != 0) {
//            $passwordcheck->andFilterWhere(['!=', 'id', $this->id]);
//        }
//        $passwordcheck = $passwordcheck->one();
//
//        if (count($passwordcheck) = NULL) {
//            $this->addError("password", "Old Password Mismatch.");
//            return FALSE;
//        }
//        
        if (!$this->save()) {
    ...
    public function updateaccount() {
        ...
        if($this->password == null || $this->password == ""){
            $this->addError('password', 'Password is missing');
            return false;
        }
